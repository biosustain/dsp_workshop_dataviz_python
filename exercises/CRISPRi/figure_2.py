...
a_data = a_data.melt(
    id_vars="time (h)", var_name="rep_cond", value_name="od_600")
a_data[["replicate", "condition"]] = a_data["rep_cond"].str.split(
    "_", n=1, expand=True)
...
# main plot
p = sns.lineplot(
    a_data, x="time (h)", y="od_600", hue="condition", errorbar="sd",
    err_style="bars", err_kws={"capsize": 4, "ecolor": "black"},
    markers=True, marker="s", markeredgewidth=0, palette=AB_COLORS,
    ax=ax
)
...
ax.legend(
    handles, ["Induced", "Uninduced"], title=None, loc="upper left",
...
ax.annotate(
    "1", (1, 0.5), xytext=(1, 1.5), fontsize=12, xycoords="data",
    horizontalalignment="center", verticalalignment="center",
    arrowprops={
        "width": 1, "headwidth": 8, "headlength": 10, "color": "black",}
)
...
b_data = b_data.melt(
    id_vars="timepoint (h)", var_name="rep_cond", value_name="yield")
b_data[["condition", "replicate"]] = b_data["rep_cond"].str.lower().str.split(
    "_", n=1, expand=True)
...
data_list = []
for bin, label in zip(
    [(2, 12), (16,), (30,)], ["2-12", "12-16", "16-30"]):
    df = b_data[b_data["timepoint (h)"].isin(bin)].copy()
    df["time_interval"] = label
    data_list.append(df)
...
total_width = 0.8
bp = sns.barplot(
    b_data, x="time_interval", y="yield", hue="condition", errorbar="sd",
    palette=AB_COLORS, capsize=0.2, width=total_width, ax=ax
)
sns.stripplot(
    b_data, x="time_interval", y="yield", hue="condition", edgecolor="black",
    linewidth=1, dodge=True, jitter=0, palette=AB_COLORS, ax=ax
)
...
ax.legend_.remove()
ax.legend(
    new_handles, ["Induced", "Uninduced"], title=None, loc="upper left",
    bbox_to_anchor=[0.01, 1.2],
    frameon=False, fontsize=15
)
...
ax.set_ylabel(
    "Yield $\\left( \\text{mol } \\text{mol}^{-1} \\right)$", labelpad=20,
    fontdict={"size": 15}
)
...
for bar_idx, y_start in zip([0, 2], [2.5, 2.7]):
    ...
    ax.text(
        (x_left + x_right) / 2, y_start + sig_height, "*",
...
# main plot
p = sns.lineplot(
    c_data, x="time (h)", y="od600", hue="variable", style="variable",
    markers=["s", "^", "o"], markersize=6, markeredgewidth=0, errorbar=None,
    dashes={var: "" for var in C_VAR_LIST},
    palette=C_COLORS, ax=ax
)

# plot the standard deviation as a shaded area
lower = sns.lineplot(
    c_data, x="time (h)", y="od_lower", hue="variable", errorbar=None,
    palette=C_COLORS, linewidth=0, ax=ax
)
upper = sns.lineplot(
    c_data, x="time (h)", y="od_upper", hue="variable", errorbar=None,
    palette=C_COLORS, linewidth=0, ax=ax
)
lines = p.get_lines()
for line_idx in range(3):
    ax.fill_between(
        ...
        color=C_COLORS[line_idx], alpha=0.2
    )
...
ax.set_yticks(
    y_tick_labels, y_tick_labels.tolist()[:-1] + [""], fontsize=15)
ax.yaxis.set_tick_params(width=2, length=10)
...
d_data = (
    d_data
        ...
        .dropna()
        .melt(
            id_vars=["time (h)", "variable"], var_name="replicate",
            value_name="conc_od"
        )
)
...
p1 = sns.lineplot(
    d_data[
        (d_data["variable"].str.contains("OD"))
        & (d_data["time (h)"] <= 25)
    ],
    x="time (h)", y="conc_od", hue="variable", errorbar="sd", err_style="bars",
    err_kws={"capsize": 4, "ecolor": "black"}, markers=True, marker="s",
    markersize=6, markeredgewidth=0, palette=D_COLORS, ax=ax1,
)
ax1.set_xlim(0, 25)
ax1.set_ylim(0, 9)
...
p2 = sns.lineplot(
    ...
    x="time (h)", y="conc_od", hue="variable", errorbar="sd",
    err_style="bars", err_kws={"capsize": 4, "ecolor": "black"},
    style="variable", markers=["^", "o", "s", "s"], markersize=6, markeredgewidth=0,
    dashes={var: "" for var in D_VAR_LIST}, palette=D_COLORS, ax=ax2
)
ax2.set_ylim(0, 130)
...
ax1.set_yticks(
    range(1, 10, 1), list(range(1, 10, 1))[:-1] + [""], fontsize=15)
...
# make an annotation with arrow to point to timepoint at 1 hour
ax2.annotate(
    "2", (2, 10), xytext=(2, 30), fontsize=12, xycoords="data",
    horizontalalignment="center", verticalalignment="center",
    arrowprops={
        "width": 1, "headwidth": 8, "headlength": 10, "color": "black",}
)
...
for i, (label, plot_fun) in enumerate(zip(
    ["a", "b", "c", "d"],
